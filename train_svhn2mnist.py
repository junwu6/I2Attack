...
import math
from utils import *
from digit_models.I2Attack import run_I2Attack
from digit_models.MDD import MDDModel
from digit_models.DANN import DANNModel
from digit_models.DAN import DANModel

# Command setting
parser = argparse.ArgumentParser(description='Domain Adaptation')
parser.add_argument('-model', type=str, default='DAN', help='model name')
...
parser.add_argument('-root_dir', type=str, default='E:/Codes/data/svhn2mnist/')
...
if __name__ == '__main__':
    ...
    if not os.path.isfile("data_preprocessed/{}.pkl".format(args.source)):
        if args.source == 'svhn':
            raw_src_loader = data_loader.load_training_svhn2mnist(args.root_dir, args.source + '.txt')
        else:
            raw_src_loader = data_loader.load_training_svhn2mnist(args.root_dir, args.source + '_train.txt')
        save_data(raw_src_loader, name=args.source)
    src_data = pickle.load(open("data_preprocessed/{}.pkl".format(args.source), "rb"))

    if not os.path.isfile("data_preprocessed/{}.pkl".format(args.target)):
        raw_tgt_loader = data_loader.load_training_svhn2mnist(args.root_dir, args.target + '_train.txt')
        save_data(raw_tgt_loader, name=args.target)
    tgt_data = pickle.load(open("data_preprocessed/{}.pkl".format(args.target), "rb"))

    if not os.path.isfile("data_preprocessed/{}_test.pkl".format(args.target)):
        raw_tgt_loader = data_loader.load_testing_svhn2mnist(args.root_dir, args.target + '_test.txt')
        save_data(raw_tgt_loader, name=args.target+'_test')
    tgt_test_data = pickle.load(open("data_preprocessed/{}_test.pkl".format(args.target), "rb"))
